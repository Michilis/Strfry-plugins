...
const WHITELIST_FILE = '/path/to/azzamo_whitelist.txt'; // Update this path

// Cache for tracking event counts per pubkey, blacklist, spam keywords, and whitelist
...
function loadWhitelist() {
    try {
        ...
        console.error('Whitelist loaded successfully.');
    } catch (e) {
...
function startProcessingEvents() {
    ...
    rl.on('line', (line) => {
        try {
            ...
            // Check if the pubkey is whitelisted
            if (whiteList.has(pubkey)) {
                console.error(`Pubkey whitelisted: ${pubkey}. Bypassing rate limit and spam filter.`);
...
// Load blacklist, spam keywords, whitelist, and start processing events
loadBlacklist();
loadSpamKeywords();
loadWhitelist();
