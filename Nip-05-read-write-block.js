...
function startProcessingEvents() {
    ...
    rl.on('line', (line) => {
        try {
            ...
            const actionType = req.type; // Can be "new" for write actions or other types for read
            ...
            if (whiteList.has(pubkey)) {
                ...
                // Force connection closure for non-whitelisted users
                res.action = 'reject';
                res.msg = 'Connection closed: not whitelisted.';
                console.log(JSON.stringify(res));

                // Explicitly close connection by returning and outputting the result
                return;
            }
